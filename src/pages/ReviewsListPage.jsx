...
//       try {
//         // ✅ التقييمات بتيجي مع بيانات الحرفيين — نجيب الحرفيين ونستخرج منهم التقييمات
//         let reviewsData;
...
//               ? `${c.crafts?.[0]?.name || 'حرفي'} محترف يقدم خدمات ممتازة` 
//               : `Professional ${c.crafts?.[0]?.name || 'craftsman'} providing excellent services`),
//             client_name: `${c.first_name || ''} ${c.last_name || ''}`.trim() || (lang === 'ar' ? 'حرفي' : 'Craftsman'),
//             client_email: c.email || '',
//             profession: c.crafts?.[0]?.name || (lang === 'ar' ? 'حرفي' : 'Craftsman'),
...
//   // ✅ تعليم التقييم كمفيد — يتم محلياً فقط (مفيش endpoint لده في الـ API حالياً)
...
const ReviewsListPage = () => {
  ...
  useEffect(() => {
    const loadReviews = async () => {
      ...
      try {
        // ✅ جلب التقييمات من endpoint الحقيقي GET /api/auth/reviews
        ...
        const reviewsData = rawReviews
          ...
          .map(r => ({
            ...
            rating: r.rating || 0,
            comment: r.comment || '',
            client_name: r.client?.name || (lang === 'ar' ? 'عميل' : 'Client'),
            craftsman_name: `${r.craftsman?.first_name || ''} ${r.craftsman?.last_name || ''}`.trim() || '',
