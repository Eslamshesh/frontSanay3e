...
const CraftsmanPostsPage = () => {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      const data = await api.getServicePosts({
        ...
        craft_id: selectedCraft || undefined,
  ...
  const handleRespond = async () => {
    ...
    try {
      ...
      // نحدث المنشور عشان يظهر إنه رد عليه
  ...
  const getUrgencyStyle = (u) => ({
    ...
    urgent: { label: isArabic ? 'عاجل جداً': 'Urgent',   bg: '#fef2f2', color: '#991b1b' },
  }[u] || { label: u, bg: '#f1f5f9', color: '#475569' });
            ...
            <div style={{
              ...
              <div>
                ...
                <select value={selectedUrgency} onChange={e => { setSelectedUrgency(e.target.value); setCurrentPage(1); }} style={inputStyle}>
                  <option value="">{isArabic ? 'الكل' : 'All'}</option>
                  <option value="low">{isArabic ? 'غير عاجل' : 'Low'}</option>
                  <option value="medium">{isArabic ? 'عادي' : 'Medium'}</option>
                  <option value="high">{isArabic ? 'عاجل' : 'High'}</option>
                  <option value="urgent">{isArabic ? 'عاجل جداً' : 'Urgent'}</option>
                </select>
              </div>
